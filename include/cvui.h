...
namespace render
{
	...
	void image(cvui_block_t& theBlock, cv::Rect& theRect, cv::Mat& theImage) {
		if (theRect.x < -theRect.width || theRect.y < -theRect.height || theRect.x >= theBlock.where.cols || theRect.y >= theBlock.where.rows) {
			return;
		}
		int xStart = 0;
		int xEnd = theImage.cols; 
		int yStart = 0;
		int yEnd = theImage.rows;

		cv::Mat newImage;

		if (theRect.x < 0) 
		{
			xStart = abs(theRect.x);
			xEnd = theImage.cols;
			theRect.x = 0;
		}
		else if (theRect.x + theRect.width > theBlock.where.cols)
		{
			xStart = 0;
			xEnd = theRect.width - (theRect.x + theRect.width - theBlock.where.cols);
		}

		if (theRect.y < 0) 
		{
			yStart = abs(theRect.y);
			yEnd = theImage.rows;
			theRect.y = 0;
		}
		else if (theRect.y + theRect.height > theBlock.where.rows)
		{
			yStart = 0;
			yEnd = theRect.height - (theRect.y + theRect.height - theBlock.where.rows);
		}
		
		newImage = theImage(cv::Range(yStart, yEnd), cv::Range(xStart, xEnd));
		theRect.height = newImage.rows;
		theRect.width = newImage.cols;
		newImage.copyTo(theBlock.where(theRect));
	}
